...
a2 = sigmoid(z2);
J = -(1/m)*sum(sum(yc.*log(a2)+(1-yc).*log(1-a2)));
...
Theta1_grad = Theta1_grad + d2'*[ones(m,1) X];
Theta2_grad = Theta2_grad + d3'*[ones(m,1) a1];
...
reglzn = (lambda/(2*m))*(sum(sum(Theta1(:,2:end).^2))+sum(sum((Theta2(:,2:end).^2))));
